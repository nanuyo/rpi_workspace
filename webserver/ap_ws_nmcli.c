...
void start_softap_mode(const char *ssid, const char *password) {
    char command[BUFFER_SIZE];

    // Set up a connection profile for the AP mode using nmcli
    snprintf(command, sizeof(command), "nmcli device wifi hotspot ifname wlan0 ssid \"%s\" password \"%s\"", ssid, password);
...
int main() {
    ...
    start_softap_mode(ap_ssid, ap_password);
